...
export const mergeVinResults = (vinNinjas: Partial<Vehicle> | null, vinGemini: Partial<Vehicle> | null): Partial<Vehicle> | null => {
  ...
  return {
    ...
    make: vinGemini?.make || vinNinjas?.make,
    model: vinGemini?.model || vinNinjas?.model,
    trim: vinGemini?.trim || vinNinjas?.trim,
    engineDisplacementL: vinGemini?.engineDisplacementL || vinNinjas?.engineDisplacementL,
    cylinders: vinGemini?.cylinders || vinNinjas?.cylinders,
    driveType: vinGemini?.driveType || vinNinjas?.driveType,
    primaryFuelType: vinGemini?.primaryFuelType || vinNinjas?.primaryFuelType,
    secondaryFuelType: vinGemini?.secondaryFuelType || vinNinjas?.secondaryFuelType,
    engineBrakeHP: vinGemini?.engineBrakeHP || vinNinjas?.engineBrakeHP,
    transmissionStyle: vinGemini?.transmissionStyle || vinNinjas?.transmissionStyle,
    bodyClass: vinGemini?.bodyClass || vinNinjas?.bodyClass,
    doors: vinGemini?.doors || vinNinjas?.doors,
    manufacturerName: vinGemini?.manufacturerName || vinNinjas?.manufacturerName,
    plantCountry: vinGemini?.plantCountry || vinNinjas?.plantCountry,
    plantState: vinGemini?.plantState || vinNinjas?.plantState,
    plantCity: vinGemini?.plantCity || vinNinjas?.plantCity,
    engineModel: vinGemini?.engineModel || vinNinjas?.engineModel,
    engineConfiguration: vinGemini?.engineConfiguration || vinNinjas?.engineConfiguration,
    electrificationLevel: vinGemini?.electrificationLevel || vinNinjas?.electrificationLevel,
    gvwr: vinGemini?.gvwr || vinNinjas?.gvwr,
    vin: vinGemini?.vin || vinNinjas?.vin,
  };
};
