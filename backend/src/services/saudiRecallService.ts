import { RecallInfo, SaudiRecallInfo } from '../types.js';
import { RecallsSaCrawler } from './recallsSaCrawler.js';
...
export class SaudiRecallService {
   ...
   */
  static async getRecallsFromOfficialSystem(vin: string, make?: string, model?: string): Promise<SaudiRecallInfo[] | null> {
    try {
  ...
  private static async scrapeRecallsFromWebsite(vin: string, make?: string, model?: string): Promise<SaudiRecallInfo[] | null> {
    try {
      ...
      // Fallback: return empty array if no real data found
      console.log('Crawler service not available or no real data found, returning no recalls.');
      return [];
    } catch (error) {
  ...
  static async getSaudiRecallStats(): Promise<{
    ...
    try {
      ...
      return {
        ...
        activeRecalls: recentRecalls.filter((r: any) => r.status === 'نشط').length,
        highRiskRecalls: recentRecalls.filter((r: any) => r.severity === 'عالية').length,
        recentRecalls: recentRecalls.filter((r: any) => {
